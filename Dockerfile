...
ENV TINI_KILL_PROCESS_GROUP=1 \ 
    ...
    TRUSTED_PLUGIN=/greengrass/v2/fleetprovisioningbyclaim-latest.jar \
    THING_POLICY_NAME=default_thing_policy_name
RUN env
RUN yum install -y shadow-utils && yum clean all
RUN groupadd ggc_group
RUN useradd -rm -d /home/ggc_user -s /bin/bash -g ggc_group ggc_user
...
# add GG Fleet Provisioning needs
RUN mkdir -p /greengrass/v2/certs
COPY certs/ /greengrass/v2/certs/
COPY AmazonRootCA1.pem /greengrass/v2/
COPY config.yaml /greengrass/v2/
COPY fleetprovisioningbyclaim-latest.jar /greengrass/v2/
